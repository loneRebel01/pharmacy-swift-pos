...
import { ArrowUpCircle, Search, Minus } from "lucide-react";
...
export default function StockOut() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      await adjustStock({ productId: selectedProduct, quantity: -quantity, type: "stock_out", notes });
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <h1 className="text-2xl font-bold flex items-center gap-2"><ArrowUpCircle className="size-6 text-red-600" /> Stock Out</h1>
      ...
      <Card className="nb-card"><CardContent className="space-y-3 pt-4">
        ...
        <Button onClick={handleSubmit} className="nb-btn-destructive text-xs" disabled={!selectedProduct || quantity <= 0}><Minus className="size-3 mr-1" /> Remove Stock</Button>
      </CardContent></Card>
    </div>
  );
}
