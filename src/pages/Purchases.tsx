...
export default function Purchases() {
  ...
  const addItem = useCallback(() => {
    ...
    const purPrice = selectedProduct.purchasePrice;
    const retPrice = selectedProduct.retailPrice;
    const cost = purPrice;
    const margin = retPrice - cost;
    const marginPercent = cost > 0 ? (margin / cost) * 100 : 0;
    const taxP = selectedProduct.tax ?? 0;
    const taxAmt = (purPrice * qty * taxP) / 100;
  ...
  const updateItemQty = useCallback((idx: number, qty: number) => {
    setItems((prev) =>
      prev.map((item, i) => {
        ...
        const taxAmt = (total * item.taxPercent) / 100;
        const finalTotal = total - item.discount + taxAmt;
        const netRate = qty > 0 ? finalTotal / qty : item.purchasePrice;
        return {
          ...item,
          quantity: qty,
          total,
  ...
  const grandTotal = subtotal - totalDiscount + totalTax + loadingExpense + freightExpense + otherExpense + additionalAmount + advanceTaxValue - additionalDiscount;
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await logActivity({ action: "Purchase created", module: "purchases", details: invoiceNumber });
    ...
    items, invoiceNumber, invoiceAmount, billNumber, date, dueDate, paymentMode,
    purchaseTax, loadingExpense, freightExpense, otherExpense, additionalAmount,
    additionalDiscount, advanceTax, advanceTaxPercent, advanceTaxValue, grandTotal,
    comments, supplierId, createPurchase, logActivity,
  ]);
  ...
  const handleHold = useCallback(async () => {
    ...
    try {
      await holdPurchase({
        data: { items, invoiceNumber, date, supplierId, paymentMode, grandTotal },
      });
      toast.success("Purchase held");
    } catch (e) {
      toast.error(String(e));
    }
  }, [items, invoiceNumber, date, supplierId, paymentMode, grandTotal, holdPurchase]);

  const handleResumeHeld = useCallback(
    async (held: typeof heldPurchases extends (infer T)[] | undefined ? T : never) => {
    ...
    <div className="space-y-4" onKeyDown={handleSearchKeyDown}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setShowHistory(!showHistory)} variant="outline" className="nb-btn-outline text-xs">
            History
          </Button>
        </div>
      </div>
      ...
      {heldPurchases && heldPurchases.length > 0 && (
        <Card className="nb-card-sm border-yellow-400">
          <CardContent className="p-3">
            <p className="text-xs font-bold mb-2">Held Purchases ({heldPurchases.length})</p>
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              <div>
                ...
                <select value={supplierId} onChange={(e) => setSupplierId(e.target.value)} className="nb-input text-xs mt-1 w-full">
              ...
              <div>
                ...
                <Input value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Invoice Amount</label>
                <Input type="number" value={invoiceAmount} onChange={(e) => setInvoiceAmount(Number(e.target.value))} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Date *</label>
                <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Due Date</label>
                <Input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Bill #</label>
                <Input value={billNumber} onChange={(e) => setBillNumber(e.target.value)} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Payment Mode</label>
                <select value={paymentMode} onChange={(e) => setPaymentMode(e.target.value)} className="nb-input text-xs mt-1 w-full">
              ...
              <div>
                ...
                <Input type="number" value={purchaseTax} onChange={(e) => setPurchaseTax(Number(e.target.value))} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Loading Expense</label>
                <Input type="number" value={loadingExpense} onChange={(e) => setLoadingExpense(Number(e.target.value))} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Freight Expense</label>
                <Input type="number" value={freightExpense} onChange={(e) => setFreightExpense(Number(e.target.value))} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Other Expense</label>
                <Input type="number" value={otherExpense} onChange={(e) => setOtherExpense(Number(e.target.value))} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Additional Amount</label>
                <Input type="number" value={additionalAmount} onChange={(e) => setAdditionalAmount(Number(e.target.value))} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Additional Discount</label>
                <Input type="number" value={additionalDiscount} onChange={(e) => setAdditionalDiscount(Number(e.target.value))} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Advance Tax %</label>
                <Input type="number" value={advanceTaxPercent} onChange={(e) => { setAdvanceTaxPercent(Number(e.target.value)); setAdvanceTaxValue((invoiceAmount * Number(e.target.value)) / 100); }} className="nb-input text-xs mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold">Advance Tax Value</label>
                <Input type="number" value={advanceTaxValue} readOnly className="nb-input text-xs mt-1 bg-muted" />
              </div>
            </div>
            <div>
              <label className="text-xs font-bold">Comments</label>
              <Input value={comments} onChange={(e) => setComments(e.target.value)} className="nb-input text-xs mt-1" placeholder="Optional comments..." />
            </div>
          </CardContent>
        )}
      </Card>
          ...
          {showQty && selectedProduct && (
            <div className="mt-2 p-3 bg-muted border-2 border-border">
              <p className="text-xs font-bold mb-1">
                ...
                <span className="ml-2 text-muted-foreground">Price: PKR {selectedProduct.purchasePrice}</span>
              </p>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-muted-foreground">
                  Total: PKR {((selectedProduct.purchasePrice * (parseInt(qtyValue) || 1))).toFixed(2)}
                </span>
              </div>
            </div>
          )}
                ...
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={17} className="text-center py-8 text-muted-foreground">
                      No products added. Press Ctrl+P to search and add products.
                    </td>
                  </tr>
                ) : (
                  items.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="text-right">
                        <input
                          ...
                          className="nb-input w-16 text-right text-xs py-0"
                          min="1"
                        />
                      </td>
                      <td className="text-right text-xs">{item.purchasePrice.toFixed(2)}</td>
                      <td className="text-right text-xs">{item.retailPrice.toFixed(2)}</td>
                      <td className="text-right text-xs">{item.cost.toFixed(2)}</td>
                      <td className="text-right text-xs text-green-700">{item.margin.toFixed(2)}</td>
                      <td className="text-right text-xs text-green-700">{item.marginPercent.toFixed(1)}%</td>
                      ...
                      <td className="text-right text-xs font-bold">{item.netRate.toFixed(2)}</td>
                      <td className="text-right text-xs font-bold">{item.finalTotal.toFixed(2)}</td>
              ...
              <table className="nb-table">
                ...
                <tbody>
                  {(purchaseList ?? []).slice(0, 20).map((p) => (
                    <tr key={p._id}>
                      <td className="text-xs font-bold">{p.invoiceNumber}</td>
                      <td className="text-xs">{p.date}</td>
                      <td className="text-xs">{(suppliers ?? []).find((s) => s._id === p.supplierId)?.name ?? "-"}</td>
                      <td className="text-right text-xs">PKR {p.totalAmount.toLocaleString()}</td>
                      <td className="text-xs">
                        <span className={`nb-badge ${p.status === "completed" ? "bg-green-100" : "bg-yellow-100"}`}>
                          {p.status}
                        </span>
                      </td>
                    </tr>
